<script setup>
/**
 * 🔹 Componente genérico de lista de DiagnosticReports
 *    - Compatible con el store de Pinia `useDiagnosticrStore`
...
import FiltersPanel from "@pages/diagnostics/filter.vue"; // filtro separado
import { useDiagnosticStore } from "@/stores/diagnostic"; // store adaptado a diagnostics
...
const resourceName = "DiagnosticReport";
...
<template>
  ...
  <!-- ============================== 
       Tabla flexible
  ============================== -->
  <FlexibleTable :store="Store" :columns="columns" icon="bx-file">
    <tr v-for="item in Store.model" :key="item.id">
      <td>{{ item.id }}</td>
      <td>{{ item.patient?.last_name || "–" }}</td>
      <td>{{ item.type || "–" }}</td>
      <td>{{ item.conclusion || item.result || "–" }}</td>
      <td>{{ item.document }}</td>
