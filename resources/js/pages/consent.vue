 ...
 * - Integrado con Pinia store `useConditionStore`
...
const resourceName = "Condición Médica";
...
// ===============================
onMounted(async () => {
  await Store.fetch(); // obtiene lista de condiciones
});
</script>
...
<template>
  ...
  <VCard class="mb-4">
    <VCardTitle class="d-flex justify-space-between align-center">
      <h2 class="text-h5 font-weight-bold">{{ resourceName }}s</h2>
  ...
  <FlexibleTable :store="Store" :columns="columns" icon="bx-heart">
    <tr v-for="item in Store.model" :key="item.id">
      ...
      <td>{{ item.recorded_date ? new Date(item.recorded_date).toLocaleDateString() : '–' }}</td>
